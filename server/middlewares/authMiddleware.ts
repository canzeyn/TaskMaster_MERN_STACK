...
const JWT_SECRET: string | undefined  = process.env.JWT_SECRET // kullanıcının oluşturduğu tokenin imzalanması için kullanılacak
 ...
 export const generateToken = (user: {id:string , role:string}) => { // token üretemesi için fonksiyondır parametre olarak girilen user  değerine özellik olarak id değeri içierdiğini ve onunda string tipinde olduğunu belirtiyoruz
   return jwt.sign({id: user.id , role: user.role } , JWT_SECRET , {expiresIn: '1d'}) // sign ile yeni bir token oluşturulur bu token 3 parametre alır  ilk kısım token içinde taşıyacağı veriyi gösteriri burada mongodb den gelen her kullanıcı için benzersiz olarak üretilen user.id değeri bu ilk veri paylaod olarak adlandırılır
